...
void osero_init(void){
	...
	int x=0;
	int y=0;
	for(y=0;y<OSERO_MAP_SIZE;y++)
	{
		for(x=0;x<OSERO_MAP_SIZE;x++)
		{
			map[y][x] = 0;
		}
	}
...
void osero_map_idou_put(void){
	...
	//下反転
	for (; map_yy < OSERO_MAP_SIZE; map_yy++) {
		if(map[map_yy+1][map_xx] == 0 || map[map_yy+1][map_xx] == iro || map_yy+2 > OSERO_MAP_SIZE){
			break;
	...
	map_yyy=map_yy=map_y;
	//右判定
	for (; map_xx < OSERO_MAP_SIZE; map_xx++) {
		if(map[map_yy][map_xx+1] == 0 || map[map_yy][map_xx+1] == iro || map_xx+2 > OSERO_MAP_SIZE){
			break;
	...
	map_yyy=map_yy=map_y;
	//右上判定
	for (; map_xx < OSERO_MAP_SIZE && map_yy < OSERO_MAP_SIZE; map_xx++,map_yy++) {
		if(map[map_yy+1][map_xx+1] == 0 || map[map_yy+1][map_xx+1] == iro ||
				map_yy+2 > OSERO_MAP_SIZE || map_xx+2 > OSERO_MAP_SIZE){
			break;
	...
	map_yyy=map_yy=map_y;
	//左上判定
	for (; map_xx > 0 && map_yy < OSERO_MAP_SIZE; map_xx--,map_yy++) {
		if(map[map_yy+1][map_xx-1] == 0 || map[map_yy+1][map_xx-1] == iro ||
				map_yy+2 > OSERO_MAP_SIZE || map_xx-2 < 4){
			break;
		}else if(map[map_yy+2][map_xx-2] == iro){
			for (; map_xxx > map_xx-2 && map_yyy < map_yy+2; map_xxx--,map_yyy++){
				map[map_yyy][map_xxx]=iro;
	...
	map_yyy=map_yy=map_y;
	//右上判定
	for (; map_xx < OSERO_MAP_SIZE && map_yy > 0; map_xx++,map_yy--) {
		if(map[map_yy-1][map_xx+1] == 0 || map[map_yy-1][map_xx+1] == iro ||
				map_yy-2 < 4 || map_xx+2 > OSERO_MAP_SIZE){
			break;
		}else if(map[map_yy-2][map_xx+2] == iro){
			for (; map_xxx < map_xx+2 && map_yyy > map_yy-2; map_xxx++,map_yyy--){
				map[map_yyy][map_xxx]=iro;
